...
export async function getUserByIdPublic(id) {
  const { rows } = await pool.query(
    `SELECT ${columns.id}, ${columns.username}, ${columns.name}, ${columns.bio}, ${columns.role}
     FROM ${table} WHERE ${columns.id} = $1`,
    [id]
  );
  return rows[0] || null;
}
...
export function countUsers() {
  return pool.query(`SELECT COUNT(*)::int AS count FROM ${table}`);
}

export function updateUserRole(userId, role) {
  return pool.query(
    `UPDATE ${table} SET ${columns.role} = $1 WHERE ${columns.id} = $2
     RETURNING ${columns.id}, ${columns.username}, ${columns.name}, ${columns.email}, ${columns.bio}, ${columns.role}`,
    [role, userId]
  );
}
